...
function createRouter(init) {
  ...
  function dispose() {
    ...
    state.fetchers.forEach((_2, key) => deleteFetcher(state.fetchers, key));
...
function RSCErrorHandler({
  ...
  if (typeof error === "object" && error && "digest" in error && typeof error.digest === "string") {
    ...
    if (redirect2) {
      ...
      return k("meta", { httpEquiv: "refresh", content: `0;url=${target}` });
    }
  }
  return children;
}
...
var MemoizedDataRoutes = N(DataRoutes2);
...
function Navigate({
  ...
  let { static: isStatic, navigator } = x(NavigationContext);
  warning(
    !isStatic,
...
async function stringify(stack) {
  ...
  while (stack.length > 0) {
    ...
    const partsForObj = (obj) => Object.keys(obj).map((k2) => `"_${flattenValue(k2)}":${flattenValue(obj[k2])}`).join(",");
    let error = null;
    switch (typeof input) {
      case "boolean":
      case "number":
      ...
      case "object": {
        ...
        if (!pluginHandled) {
          ...
          if (isArray) {
            ...
            if (input.size > 0) {
              str[index] = `["${TYPE_MAP}",${[...input].flatMap(([k2, v]) => [flattenValue(k2), flattenValue(v)]).join(",")}]`;
            } else {
              str[index] = `["${TYPE_MAP}"]`;
            }
...
function Links({ nonce, crossOrigin }) {
  ...
  return k(S, null, typeof criticalCss === "string" ? k(
    ...
    ({ key, link }) => isPageLinkDescriptor(link) ? k(
      PrefetchPageLinks,
      {
        key,
        nonce,
...
function PrefetchPageLinks({ page, ...linkProps }) {
  ...
  if (rsc) {
    return k(RSCPrefetchPageLinksImpl, { page, matches, ...linkProps });
  }
...
function RSCPrefetchPageLinksImpl({
  ...
  return k(S, null, dataHrefs.map((href2) => k("link", { key: href2, rel: "prefetch", as: "fetch", href: href2, ...linkProps })));
}
...
var Link = D(
  function LinkWithRef({
    ...
    let { basename, navigator, useTransitions } = x(NavigationContext);
...
var Form = D(
  ({
    ...
    let { useTransitions } = x(NavigationContext);
